...
#
# Name:
#    deactivate_matlab.sh -  script file for invoking the MATLAB
#    deactivation app.
#
#    Usage:deactivate_matlab.sh [-h|help]|"
#                               [-javadir <directory>] |"
#                               [-root <directory>] |"
#                               [-test] | [-v|-verbose] | [-log <log file>"
...
#                              -log  <filename>  - Override log file path."
#
...
#========================= java_launcher_config (end) ==============================
   checkDisplay () {
    ...
    if [ "$DISPLAY" = "" ]; then
       if [ $ISMAC -eq 0 ]; then
           echo "---------------------------------------------------------------------------"
           echo "Error: There is no display set.  The deactivation process"
           echo "requires that an X display be set."
           echo "---------------------------------------------------------------------------"
           exit 1
       fi
    fi
...
#
    ...
    logfile=
    arglist=
    LAUNCHER_PROPERTIES=java/config/deactivation/launcher.config
    while [ "$stat" = "OK" -a  $# -gt 0 ]; do
        case "$1" in
            ...
            -log)
                if [ $# -eq 1 ]; then
                    echo "A log file must be specified with -log."
                    stat=""
                else
                    shift
                    logfile=$1;
            ...
            -test)
                ...
                ;;
            -psn*)
                # This argument is automatically passed in by Apple. Ignore.
                ;;
            -*)
    	found=0
                arch=$ARCH
...
#++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
    echo ""
    echo "   Usage: deactivate_matlab.sh [-h|help]|"
    echo "                             [-javadir <directory>] | [-root <directory>] |"
    echo "                             [-test] | [-v|-verbose]  | [-log <log file>"
    echo ""
    ...
    echo "    -log <filename>            - Override default log file name."
    echo ""
...
#   deactivation never copies, always use $ROOT
...
#   Set up java command to run.
    if [ $ISMAC -eq 1 ]; then
        XTRAFLAGS="-d64 -Xdock:name=\"Deactivate MATLAB\" -Xdock:icon=\"$ROOT/Deactivate.app/Contents/Resources/membrane.icns\""
    fi
...
#
#
#   Run the java command to start the deactivation app
    if [ "$TESTONLY" != "1" ]; then
        eval $java_cmd
    fi
